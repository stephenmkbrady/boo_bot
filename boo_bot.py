...
• Version: Simplified with dynamic naming"""
                ...
                if len(command) > len(command_start):
                    url = command[len(command_start):].strip()
                    if url:
                        await self.youtube_processor.handle_youtube_subs(room.room_id, url, is_edit, send_message_func=self.send_message, send_file_attachment_func=self.send_file_attachment)
                    else:
